...
minio_client_configure_local() {
    ...
    minio_client_execute config host add local "${MINIO_SERVER_SCHEME}://${MINIO_SERVER_HOST:-localhost}:${MINIO_SERVER_PORT_NUMBER}" "${MINIO_SERVER_ROOT_USER}" "${MINIO_SERVER_ROOT_PASSWORD}" >/dev/null 2>&1
}
